...
async function requireUser(req: Request) {
  ...
  if (error || !data?.user) {
    return { supabase, user: null as any, error: "Not authenticated" };
  }
...
export async function GET(req: Request) {
  const { supabase, user, error } = await requireUser(req);
...
export async function POST(req: Request) {
  ...
  if (!nameRaw) return NextResponse.json({ error: "Missing name" }, { status: 400 });
    ...
    .upsert(
      {
        ...
        quantity: quantityText, // schema stores quantity as text
        unit: null,
        checked: false,
        dismissed: false,
        source_type: "manual",
 ...
 * BULK PATCH
 * Body:
 * { ids: string[], patch: { checked?: boolean, dismissed?: boolean, quantity?: number|string, name?: string } }
 */
export async function PATCH(req: Request) {
  ...
  if (!patch) return NextResponse.json({ error: "Missing patch" }, { status: 400 });
