...
import { matchIngredientToStorage } from "@/lib/ingredientMatch";
...
export default function ShoppingListPage() {
  ...
  const duplicateGroupsRaw = useMemo(() => {
    ...
    for (const [_key, list] of groupMap.entries()) {
      ...
      for (const s of storageSignatures) {
        const result = matchIngredientToStorage(display, s.items);
        if (result?.matched) {
          matches = s.items;
          break;
        }
